...
def pick_strict_side_angle(dx, dy):
    """Determine a docking angle based on the dominant axis direction."""
    if abs(dx) >= abs(dy):
...
def update_block_visualization(block_body_id, block_center_sphere):
    """Update the block's center sphere and orientation frame."""
...
def load_robot(urdf_path):
    """Load the robot and return its ID and joint mapping."""
    try:
...
def load_block(block_urdf_path):
    """Load the block at a random position and return its ID and center sphere ID."""
...
def create_destination_visual():
    """Create and return the destination target visual."""
...
def drive_to_target(robot_id, joint_dict, target, forward_speed, pid_controller, dt, force, stop_distance=0.12):
    """
    Compute motion commands to drive the robot toward a given target.
    Returns the current distance and angle error.
    """
    base_pos, base_orn = p.getBasePositionAndOrientation(robot_id)
...
def load_and_control_urdf(urdf_path, block_urdf_path):
    ...
    dt = 1 / 120.0
    # Initialize PID controller for angular adjustments.
    ...
    try:
        while True:
            ...
            if current_state == STATE_IDLE:
                ...
                # Drive toward the block's center.
                ...
                distance, angle_error = drive_to_target(robot_id, joint_dict, target,
                                                        ...
                                                        stop_distance=final_approach_distance + predock_offset)
                ...
                # Relative Orientation Alignment with docking logic.
                pos_block, orn_block = p.getBasePositionAndOrientation(block_body_id)
                rot_mat = p.getMatrixFromQuaternion(orn_block)
                ...
                print(f"[DEBUG] FINAL_ALIGN_BLOCK: desired_angle={math.degrees(desired_angle):.1f}°, rover_yaw={math.degrees(rover_yaw):.1f}°, angle_error={math.degrees(angle_error):.1f}°")
