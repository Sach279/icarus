...
class PIDController:
    ...
    def compute(self, error, dt):
        ...
        output = max(min(output, MAX_ROTATION_VELOCITY), -MAX_ROTATION_VELOCITY) # Clamp final output
        return output
...
def pick_strict_side_angle(dx, dy):
    if not (is_valid_coordinate(dx) and is_valid_coordinate(dy)): return 0.0 # Default angle if input invalid
...
def load_blocks(block_urdf_path, num_to_load):
    ...
    for i in range(num_to_load):
        ...
        except Exception as e: print(f"!!!!!!!!!!!!!!!!!!\n[ERROR] FAILED TO LOAD BLOCK {i+1} URDF:", e); traceback.print_exc(); print("!!!!!!!!!!!!!!!!!!"); time.sleep(10); return None, None, None
...
class BuilderBot:
    def __init__(self, robot_id, joint_dict, block_viz_dict, initial_block_states, target_vis_id, dest_vis_id):
        ...
        self.block_viz_dict = block_viz_dict; self.block_states = initial_block_states
        ...
        self.placed_block_count = 0; self.final_drive_start_time = None
        self.final_drive_target_yaw = 0.0; self.retreat_yaw = 0.0
    ...
    def check_stability(self, angle_aligned, robot_id):
        try:
            ...
            angular_velocity_low = abs(ang_vel[2]) < ANGULAR_VELOCITY_STABILITY_THRESHOLD
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            ...
            if self.target_block_id is None or target_status != 'AVAILABLE': print(f"[WARN] APPROACH_BLOCK: Invalid target block (ID: {self.target_block_id}, Status: '{target_status}'). Returning to FIND_BLOCK."); self.current_state = STATE_FIND_BLOCK; self.target_block_id = None; return target
            try:
             ...
             # (No change - stability check now includes angular velocity)
             ...
             try:
                ...
                angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
                pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP); # <<< Uses clamped output
            ...
            # (No change - already includes timeout and yaw correction)
            ...
            if time.time() - self.final_drive_start_time > FINAL_DRIVE_TIMEOUT:
                ...
                try:
                    ...
                    if distance > DOCKING_DISTANCE:
                        ...
                        pid_correction = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP) # <<< Uses clamped output
            ...
            # <<< Use the modified Y-axis alignment strategy >>>
            ...
            if not aligned:
             ...
             # (No change - already includes yaw correction)
            target = self.current_placement_target; dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]; distance = math.hypot(dx, dy)
            ...
            if distance_retreated < RETREAT_DISTANCE:
                # Apply backward speed
                left_velocity = -RETREAT_SPEED
                right_velocity = -RETREAT_SPEED
                # Add gentle correction to maintain retreat angle
