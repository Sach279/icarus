...
FINAL_DRIVE_TIMEOUT = 10.0 # Timeout for FINAL_DRIVE_BLOCK in seconds
...
def are_valid_coordinates(*coords):
    # Check if coords is empty or None first
    if not coords:
        return False
    # Flatten the list of points/vectors and check each component
    flat_coords = [c for point in coords if point is not None for c in point]
    if not flat_coords: # Handle case where all inputs were None
        return False
...
def simulate_lidar(robot_id, num_rays=36, ray_length=5.0, fov_deg=140):
    try:
        ...
        if not are_valid_coordinates(base_pos): return # Exit if robot pose invalid
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            ...
            for block_id, status in self.block_states.items():
                if status == 'AVAILABLE': available_blocks_exist = True; # Corrected indent
                try:
                    pos_block, _ = p.getBasePositionAndOrientation(block_id)
                    if not are_valid_coordinates(pos_block) or status != 'AVAILABLE': continue # Also check status here
            ...
            # --- Debug prints added ---
            print(f"  [DEBUG_APPROACH] Entering state. Target Block ID = {self.target_block_id}")
            target_status = self.block_states.get(self.target_block_id, 'NOT_FOUND') # Use get with default
            print(f"  [DEBUG_APPROACH] Status for block {self.target_block_id} = '{target_status}'")
            # --- End of debug prints ---
            ...
            if self.target_block_id is None or target_status != 'AVAILABLE':
                 ...
                 self.target_block_id = None
                 return target # Return target for camera here too
             ...
             try:
                ...
                if not aligned: left_velocity = -pid_output * ROTATION_SPEED_SCALE; right_velocity = pid_output * ROTATION_SPEED_SCALE; self.reset_stability_timer()
                    ...
                    if self.check_stability(aligned):
                        ...
                        self.final_drive_start_time = None # Reset timer before entering next state
            ...
            if time.time() - self.final_drive_start_time > FINAL_DRIVE_TIMEOUT:
                ...
                if self.target_block_id is not None: self.block_states[self.target_block_id] = 'AVAILABLE' # Make available again
            ...
            if self.dock_constraint is not None:
                ...
                if self.target_block_id is not None:
                    ...
                    self.target_block_id = None # <<< Important: Clear target ID after placing
    ...
    try:
        while p.isConnected():
            ...
            if builder_bot.dock_constraint is not None:
                try:
                    dest_pos = builder_bot.current_placement_target # Use calculated placement target
