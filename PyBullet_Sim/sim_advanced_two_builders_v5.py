...
class BuilderBot:
    ...
    # <<< Updated Collision Check Method >>>
    def check_robot_collision(self, base_pos, all_robots):
        """Checks for collision risk with OTHER robots. Returns True if THIS robot should stop."""
            ...
            try:
                ...
                if dist_sq < ROBOT_COLLISION_THRESHOLD**2:
                    if self.robot_instance_id > other_bot.robot_instance_id:
                        # print(f"[WARN] Robot {self.robot_instance_id}: Collision risk with lower ID Robot {other_bot.robot_instance_id}. Stopping.") # Reduce spam
                        return True # This robot (higher ID) should stop
                    # else: print(f"[DEBUG] Robot {self.robot_instance_id}: Potential collision with higher ID Robot {other_bot.robot_instance_id}. Continuing (priority).")
    ...
    def run_step(self, all_robots):
        ...
        should_stop_for_collision = False # Default to not stopping
        ...
        if self.current_state == STATE_IDLE:
            ...
            # (Revised logic from previous step)
        ...
        # <<< New State >>>
            ...
            # print(f"[DEBUG] Robot {self.robot_instance_id}: WAITING_FOR_DEST: Checking lock...") # Reduce spam
            current_locker = self.shared_data['destination_locked_by']
            if current_locker == -1:
                 self.shared_data['destination_locked_by'] = self.robot_instance_id
            ...
            # else: Stay in this state and do nothing
            left_velocity = right_velocity = 0.0
        ...
        # Only check if actually trying to move
