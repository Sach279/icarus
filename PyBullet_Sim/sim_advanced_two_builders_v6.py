...
class BuilderBot:
            ...
            try:
                ...
                if dist_sq < ROBOT_COLLISION_THRESHOLD**2:
                    if self.robot_instance_id > other_bot.robot_instance_id:
                        # print(f"[WARN] Robot {self.robot_instance_id}: Collision risk with lower ID Robot {other_bot.robot_instance_id}. Stopping.")
                        return True
    ...
    def update_target_line(self, base_pos):
        ...
        # Draw new line if robot has a valid target block
        if self.target_block_id is not None and self.current_state >= STATE_APPROACH_BLOCK and self.current_state < STATE_LOCATE_DEST:
            try:
                ...
                if are_valid_coordinates(base_pos, target_pos):
                    self.target_debug_line = p.addUserDebugLine(
                        base_pos,
                        [target_pos[0], target_pos[1], base_pos[2]], # Keep line horizontal
                        lineColorRGB=[0, 1, 0], # Green line
                        lineWidth=2,
                        lifeTime=SIMULATION_TIMESTEP * 3 # Make it persist slightly longer
                    )
            except p.error:
    ...
    def run_step(self, all_robots):
        ...
        # --- Clear target line at the beginning of the step ---
        self.update_target_line(base_pos) # Will remove old line / draw new one if applicable
        ...
        if self.current_state == STATE_IDLE:
            ...
            # (No change in logic, but will call update_target_line after finding target)
            ...
            left_velocity = right_velocity = 0.0
             ...
             # (Logic unchanged - uses shared index)
            ...
            # (Includes destination lock check)
        ...
        if self.current_state not in [STATE_IDLE, STATE_FIND_BLOCK, STATE_ALL_DELIVERED, STATE_WAITING_FOR_DEST]: # Only check if moving/aligning
             if abs(left_velocity) > 0.01 or abs(right_velocity) > 0.01: # Check if movement is intended
    ...
    try:
        while p.isConnected():
            ...
            for i, bot in enumerate(robots):
                try:
                    # Pass the full list of robots to run_step
            ...
            # (Minor update to placement count text)
            try:
                 ...
                 # Display placement count near robot 0
