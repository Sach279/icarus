...
PLACEMENT_OFFSET = 0.11 # Distance between placed blocks (slightly > 0.1m)
...
FINAL_APPROACH_DISTANCE = 0.12 # General distance for final moves
DOCKING_DISTANCE = 0.11 # How close to get before docking/undocking
...
def simulate_lidar(robot_id, num_rays=36, ray_length=5.0, fov_deg=140):
    ...
    except p.error: pass # Ignore lidar errors
...
def load_robot(urdf_path):
    ...
    try:
        ...
        print("!!!!!!!!!!!!!!!!!!\n[ERROR] FAILED TO LOAD ROBOT URDF:", e); traceback.print_exc(); print("!!!!!!!!!!!!!!!!!!")
        time.sleep(10); return None, None
...
def load_blocks(block_urdf_path, num_to_load):
    ...
    for i in range(num_to_load):
        ...
        try:
            ...
            print("!!!!!!!!!!!!!!!!!!\n[ERROR] FAILED TO LOAD BLOCK {i+1} URDF:", e); traceback.print_exc(); print("!!!!!!!!!!!!!!!!!!")
            time.sleep(10); return None, None, None
    print(f"[DEBUG] Successfully loaded {len(blocks)} blocks.")
    return blocks, block_states

def create_destination_visual():
    dest_shape_id = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.055,0.055,0.055], rgbaColor=[0,0,1,0.5]) # Smaller blue cube
...
def initialize_simulation():
    ...
    try:
        client_id = p.connect(p.GUI); assert client_id >= 0, "Failed connection"
...
class BuilderBot:
    def __init__(self, robot_id, joint_dict, block_viz_dict, initial_block_states, target_vis_id, dest_vis_id):
        ...
        self.base_destination = [0.0, -2.0] # Initial base destination
        self.current_placement_target = [0.0, 0.0] # Calculated target for current block
        self.placed_block_count = 0 # Counter for sequential placement

        # Sliders now control the BASE destination for the first block
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            if self.stability_timer is None: self.stability_timer = time.time()
            if time.time() - self.stability_timer >= 1.0:
                self.current_state = STATE_FIND_BLOCK; self.reset_stability_timer(); self.angle_pid.reset()
            ...
            for block_id, status in self.block_states.items():
                if status == 'AVAILABLE':
                    ...
                    except p.error: continue # Skip invalid block ID
            if closest_block_id != -1:
                ...
                if abs(angle_error) < FIND_BLOCK_ANGLE_THRESHOLD:
                    print(f"[INFO] FIND_BLOCK: Aligned with closest block {self.target_block_id}. Transitioning to APPROACH_BLOCK")
                    ...
                    left_velocity = -pid_output * ROTATION_SPEED_SCALE * 0.5; right_velocity = pid_output * ROTATION_SPEED_SCALE * 0.5
            elif available_blocks_exist: print("[WARN] FIND_BLOCK: Available blocks exist, but failed to find closest. Rotating slowly."); left_velocity = -0.5; right_velocity = 0.5
            ...
            if closest_block_id == -1: left_velocity = right_velocity = 0.0 # Stop if no target
            ...
            if self.target_block_id is None or self.block_states.get(self.target_block_id) != 'AVAILABLE':
                 print("[WARN] APPROACH_BLOCK: Invalid target block. Returning to FIND_BLOCK."); self.current_state = STATE_FIND_BLOCK; self.target_block_id = None; return target
            try:
                ...
                if distance > APPROACH_BLOCK_STOP_DISTANCE:
                    ...
                    print("[INFO] APPROACH_BLOCK: Reached offset point. Transition to ALIGN_BLOCK"); self.current_state = STATE_ALIGN_BLOCK
                    self.angle_pid.reset(); self.reset_stability_timer(); p.resetBasePositionAndOrientation(self.target_vis_id, [0,0,-1], [0,0,0,1]); left_velocity = right_velocity = 0.0
            ...
            try:
                ...
                if not aligned: left_velocity = -pid_output * ROTATION_SPEED_SCALE; right_velocity = pid_output * ROTATION_SPEED_SCALE; self.reset_stability_timer()
                    ...
                    left_velocity = right_velocity = 0.0
                    if self.check_stability(aligned): print("[INFO] ALIGN_BLOCK: Alignment stable. Transition to FINAL_DRIVE_BLOCK"); self.current_state = STATE_FINAL_DRIVE_BLOCK; self.angle_pid.reset()
            ...
            try:
                ...
                if distance > DOCKING_DISTANCE:
                    desired_angle = rover_yaw; angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
                    pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP)
                    left_velocity = FINAL_DRIVE_SPEED - pid_output; right_velocity = FINAL_DRIVE_SPEED + pid_output
                else: print("[INFO] FINAL_DRIVE_BLOCK: Reached docking distance. Transition to DOCKING."); self.current_state = STATE_DOCKING; left_velocity = right_velocity = 0.0
            except p.error as e: print(f"[ERROR] PyBullet error in FINAL_DRIVE_BLOCK: {e}. Resetting."); self.current_state = STATE_FIND_BLOCK; self.target_block_id = None
            ...
            if self.dock_constraint is None:
                try:
                    ...
                    print(f"[INFO] DOCKING: Constraint created for block {self.target_block_id}. Block attached."); print("[INFO] DOCKING: Transitioning to LOCATE_DEST.")
                    self.current_state = STATE_LOCATE_DEST; self.angle_pid.reset()
            ...
            # Read BASE destination from sliders
            base_dest_x = p.readUserDebugParameter(self.base_dest_x_slider)
            base_dest_y = p.readUserDebugParameter(self.base_dest_y_slider)
            self.base_destination = [base_dest_x, base_dest_y]

            # Calculate the specific target for THIS block based on count
            target_x = self.base_destination[0] + self.placed_block_count * PLACEMENT_OFFSET
            target_y = self.base_destination[1]
            self.current_placement_target = [target_x, target_y]
            target = self.current_placement_target # For camera

            # Update blue destination cube visualization to the calculated spot
            p.resetBasePositionAndOrientation(self.dest_vis_id, [target[0], target[1], 0.05], [0,0,0,1]) # Place blue cube slightly above ground
            ...
            target = self.current_placement_target # Use calculated target
            dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]; distance = math.hypot(dx, dy)
            desired_angle = math.atan2(dy, dx); angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
            pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP)
            ...
            target = self.current_placement_target # Use calculated target
            dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]
            desired_angle = pick_strict_side_angle(dx, dy); angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
            pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP); aligned = abs(angle_error) < ALIGNMENT_STABILITY_THRESHOLD
            if not aligned: left_velocity = -pid_output * ROTATION_SPEED_SCALE; right_velocity = pid_output * ROTATION_SPEED_SCALE; self.reset_stability_timer()
            else:
                left_velocity = right_velocity = 0.0
                if self.check_stability(aligned): print("[INFO] ALIGN_DEST: Alignment stable. Transition to FINAL_DRIVE_DEST"); self.current_state = STATE_FINAL_DRIVE_DEST; self.angle_pid.reset()

        elif self.current_state == STATE_FINAL_DRIVE_DEST:
            target = self.current_placement_target # Use calculated target
            dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]; distance = math.hypot(dx, dy)
            if distance > DOCKING_DISTANCE - 0.01: # Drive slightly closer than docking dist
                desired_angle = rover_yaw; angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
                pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP)
                left_velocity = FINAL_DRIVE_SPEED - pid_output; right_velocity = FINAL_DRIVE_SPEED + pid_output
            ...
            if self.dock_constraint is not None:
                ...
                if self.target_block_id is not None:
                    ...
                    self.placed_block_count += 1 # Increment placement counter
            ...
            if distance_retreated < RETREAT_DISTANCE: left_velocity = -RETREAT_SPEED; right_velocity = -RETREAT_SPEED
            else:
                print("[INFO] RETREAT: Retreat complete. Transitioning back to FIND_BLOCK."); self.current_state = STATE_FIND_BLOCK
                self.retreat_start_pos = None; self.angle_pid.reset(); left_velocity = right_velocity = 0.0
        ...
        if target is None and self.target_block_id is not None and self.block_states.get(self.target_block_id) == 'HELD':
            target = self.current_placement_target # Keep camera on calculated placement target while holding
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    if block_visualizations is None: return
    ...
    try:
        while p.isConnected():
            ...
            except p.error as e: print(f"[ERROR] PyBullet error during bot step (state {builder_bot.current_state}): {e}"); builder_bot.current_state = STATE_IDLE; continue # Skip rest of loop if error
            ...
            if builder_bot.dock_constraint is not None:
                try:
                    dest_pos, _ = p.getBasePositionAndOrientation(builder_bot.dest_vis_id) # Get current pos of blue cube
                ...
                except p.error: transport_line_id = None # Handle if dest_vis_id is invalid
            ...
            try:
                 ...
                 p.addUserDebugText(imu_text, [base_pos[0]-0.3, base_pos[1]+0.3, base_pos[2]+0.5], [1,1,1], 1.0, SIMULATION_TIMESTEP*2)
                 p.addUserDebugText(state_text, [base_pos[0]-0.3, base_pos[1]+0.2, base_pos[2]+0.5], [1,1,0], 1.0, SIMULATION_TIMESTEP*2)
                 p.addUserDebugText(blocks_text, [base_pos[0]-0.3, base_pos[1]+0.1, base_pos[2]+0.5], [0,1,1], 1.0, SIMULATION_TIMESTEP*2) # Cyan block count
            except p.error: pass
