...
ALIGNMENT_STABILITY_THRESHOLD = math.radians(1.5)
...
# --- Helper Functions ---
# (is_valid_coordinate, are_valid_coordinates, simulate_lidar, pick_strict_side_angle,
#  set_motor_speeds, update_camera_view, update_all_block_visualizations - unchanged)
...
class BuilderBot:
    def __init__(self, robot_instance_id, robot_id, joint_dict, block_viz_dict, shared_block_states, shared_data, target_vis_id, dest_vis_id):
        ...
        self.base_destination = [0.0, -2.0]; self.current_placement_target = [0.0, 0.0]
        # Uses shared_data['placed_count']
    ...
    def get_state_name(self):
        states = ["IDLE","FIND_BLOCK","APPROACH_BLOCK","ALIGN_BLOCK","FINAL_DRIVE_BLOCK","DOCKING","LOCATE_DEST","APPROACH_DEST","ALIGN_DEST","FINAL_DRIVE_DEST","UNDOCKING","RETREAT","ALL_DELIVERED"]
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            if self.stability_timer is None: self.stability_timer = time.time()
            if time.time() - self.stability_timer >= 1.0 + random.uniform(0, 0.5): self.current_state = STATE_FIND_BLOCK; self.reset_stability_timer(); self.angle_pid.reset(); print(f"[INFO] Robot {self.robot_instance_id}: IDLE complete -> FIND_BLOCK")
            left_velocity = right_velocity = 0.0

        elif self.current_state == STATE_FIND_BLOCK:
            # <<< Rewritten Logic for Multi-Robot >>>
            my_target_id = -1
            rotate_only = False

            # --- Robot 0 Calculates Targets ---
            if self.robot_instance_id == 0:
                # Reset assignment flag at the start of calculation
                self.shared_data['block_targets_assigned'] = False
                available_blocks = [] # List of (dist_sq, block_id)
                for block_id, status in self.block_states.items():
                    if status == 'AVAILABLE':
                        try:
                            ...
                            available_blocks.append((dist_sq, block_id))
                        except p.error: continue

                available_blocks.sort() # Sort by distance squared

                # Assign targets
                self.shared_data['target_for_robot_0'] = available_blocks[0][1] if len(available_blocks) > 0 else -1
                self.shared_data['target_for_robot_1'] = available_blocks[1][1] if len(available_blocks) > 1 else -1
                self.shared_data['block_targets_assigned'] = True # Signal calculation complete
                print(f"[DEBUG] Robot 0 calculated targets: R0={self.shared_data['target_for_robot_0']}, R1={self.shared_data['target_for_robot_1']}")

            # --- All Robots Read Assigned Target ---
            if self.shared_data.get('block_targets_assigned', False):
                ...
                if my_target_id != -1 and self.block_states.get(my_target_id) == 'AVAILABLE':
                    self.target_block_id = my_target_id # Officially claim this target for now
                    pos_block, _ = p.getBasePositionAndOrientation(self.target_block_id)
                    target = [pos_block[0], pos_block[1]] # For camera/debug
                    dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]
                    desired_angle = math.atan2(dy, dx)
                    angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi

                    if abs(angle_error) < FIND_BLOCK_ANGLE_THRESHOLD:
                        print(f"[INFO] Robot {self.robot_instance_id}: FIND_BLOCK: Aligned with assigned block {self.target_block_id}. -> APPROACH_BLOCK")
                        self.current_state = STATE_APPROACH_BLOCK; self.angle_pid.reset()
                        # Reset shared flag only when BOTH might have transitioned (simplification: reset when R0 finds target assigned)
                        if self.robot_instance_id == 0:
                             self.shared_data['block_targets_assigned'] = False
                    else:
                        # Rotate towards assigned target
                        ...
                        rotate_only = True # Flag that we are only rotating
                else:
                    # No valid target assigned or target was taken/invalid
                    self.target_block_id = None
                    # Check if ANY blocks are left at all
                    if any(status == 'AVAILABLE' for status in self.block_states.values()):
                        # print(f"[DEBUG] Robot {self.robot_instance_id}: FIND_BLOCK: No valid target assigned or target taken. Waiting/Rotating slowly.")
                        # Optional: Rotate slowly if waiting for Robot 0 or if target gone
                        if not self.shared_data.get('block_targets_assigned', False) and self.robot_instance_id != 0:
                             pass # Robot 1 waits if R0 hasn't finished calculating
                        else:
                            left_velocity = -0.3 # Slow rotation if target invalid
                            right_velocity = 0.3
                            rotate_only = True
                    else:
                         print(f"[INFO] Robot {self.robot_instance_id}: FIND_BLOCK: No available blocks remain. -> ALL_DELIVERED.")
                         self.current_state = STATE_ALL_DELIVERED
            else:
                 # Robot 1 waiting for Robot 0's calculation
                 if self.robot_instance_id != 0:
                      # print(f"[DEBUG] Robot {self.robot_instance_id}: FIND_BLOCK: Waiting for target assignment.")
                      pass # Stay still
                 else:
                     # Robot 0 should always calculate, this path ideally not taken unless error
                     print(f"[WARN] Robot {self.robot_instance_id}: FIND_BLOCK: Target assignment flag not set unexpectedly.")


            if not rotate_only: # Stop motors if not rotating
                left_velocity = right_velocity = 0.0
            ...
            if self.target_block_id is None or target_status != 'AVAILABLE':
                 print(f"[WARN] Robot {self.robot_instance_id}: APPROACH_BLOCK: Target block {self.target_block_id} no longer available (Status: '{target_status}'). -> FIND_BLOCK.")
                 self.current_state = STATE_FIND_BLOCK; self.target_block_id = None; return target # Go back to find another
            try:
                ...
                if distance > APPROACH_BLOCK_STOP_DISTANCE:
                    ...
                    print(f"[INFO] Robot {self.robot_instance_id}: APPROACH_BLOCK: Reached offset point. -> ALIGN_BLOCK")
             ...
             target_status = self.block_states.get(self.target_block_id, 'NOT_FOUND') # <<< Check shared state
             if self.target_block_id is None or target_status != 'AVAILABLE': print(f"[WARN] Robot {self.robot_instance_id}: ALIGN_BLOCK: Target block {self.target_block_id} no longer available. -> FIND_BLOCK."); self.current_state = STATE_FIND_BLOCK; return target
             try:
                # (Alignment logic unchanged - uses stability check with velocity)
                pos_block, orn_block = p.getBasePositionAndOrientation(self.target_block_id)
                if not are_valid_coordinates(pos_block): raise p.error("Invalid block coordinates")
                rot_mat = p.getMatrixFromQuaternion(orn_block); local_x = np.array([rot_mat[0], rot_mat[3]]); local_y = np.array([rot_mat[1], rot_mat[4]])
                vec = np.array([base_pos[0]-pos_block[0], base_pos[1]-pos_block[1]]); norm = np.linalg.norm(vec); vec_norm = vec/norm if norm > 1e-3 else np.array([1,0])
            ...
            if time.time() - self.final_drive_start_time > FINAL_DRIVE_TIMEOUT:
                ...
                if self.target_block_id is not None and self.target_block_id in self.block_states: self.block_states[self.target_block_id] = 'AVAILABLE' # <<< Update shared state
            ...
            if self.dock_constraint is None:
                try:
                    ...
                    self.block_states[self.target_block_id] = 'HELD' # <<< Update shared state
                    print(f"[INFO] Robot {self.robot_instance_id}: DOCKING: Constraint created for block {self.target_block_id}. Block attached."); print(f"[INFO] Robot {self.robot_instance_id}: DOCKING: -> LOCATE_DEST."); self.current_state = STATE_LOCATE_DEST; self.angle_pid.reset()
                    # Reset the shared assignment flag after successful docking
                    if self.robot_instance_id == 0: self.shared_data['block_targets_assigned'] = False
            ...
            current_placed_count = self.shared_data['placed_count'] # <<< Read shared count
            target_x = self.base_destination[0] + current_placed_count * PLACEMENT_OFFSET; target_y = self.base_destination[1]
            ...
            print(f"[INFO] Robot {self.robot_instance_id}: LOCATE_DEST: Base Dest ({base_dest_x:.2f}, {base_dest_y:.2f}), Shared Placed Count {current_placed_count}, Target ({target_x:.2f}, {target_y:.2f}). -> APPROACH_DEST.")
            ...
            if distance > FINAL_APPROACH_DISTANCE:
                speed_factor = max(0.1, math.cos(angle_error)**2); current_speed = FORWARD_SPEED * speed_factor
                left_velocity = current_speed - pid_output; right_velocity = current_speed + pid_output
            else: print(f"[INFO] Robot {self.robot_instance_id}: APPROACH_DEST: Reached destination proximity. -> ALIGN_DEST"); self.current_state = STATE_ALIGN_DEST; self.angle_pid.reset(); self.reset_stability_timer(); left_velocity = right_velocity = 0.0
            ...
            if self.dock_constraint is not None:
                ...
                if self.target_block_id is not None:
                    self.block_states[self.target_block_id] = 'PLACED' # <<< Update shared state
                    self.shared_data['placed_count'] += 1 # <<< Increment shared counter
            ...
            left_velocity = right_velocity = 0.0 # Stay idle
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    shared_data = {
        ...
        'block_targets_assigned': False # Flag for FIND_BLOCK coordination
    }
    ...
    try:
        while p.isConnected():
            ...
            for i, bot in enumerate(robots):
                try:
                    # Get base_pos for the specific bot being updated
                    bot_base_pos, bot_base_orn = p.getBasePositionAndOrientation(bot.robot_id)
                    current_camera_target = bot.run_step()
                    # Update camera focus logic
                    if bot.current_state != STATE_IDLE and bot.current_state != STATE_ALL_DELIVERED:
                        active_bot_index = i # Focus on the last active bot
                        camera_target_overall = current_camera_target
                    elif i == 0 and camera_target_overall is None: # Default focus if R0 is idle and no other active
                         camera_target_overall = current_camera_target
            ...
            # --- Update Visualizations (Once per frame) ---
            try:
                 update_all_block_visualizations(robots[0].block_viz_dict)
                 if auto_camera:
                     # Ensure active_bot_index is valid before getting pose
            ...
            # --- IMU/State/Blocks Text (Show for first robot) ---
            try:
                 bot0 = robots[0]
                 base_pos, base_orn = p.getBasePositionAndOrientation(bot0.robot_id)
                 roll, pitch, rover_yaw = p.getEulerFromQuaternion(base_orn)
                 imu_text = f"R0 IMU: R:{math.degrees(roll):.0f} P:{math.degrees(pitch):.0f} Y:{math.degrees(rover_yaw):.0f}"
                 state_text = f"R0 State: {bot0.get_state_name()}"
                 # Show state for Robot 1 as well
                 state_text_r1 = f"R1 State: {robots[1].get_state_name()}" if len(robots) > 1 else ""
                 blocks_text = f"Placed: {bot0.shared_data['placed_count']}/{NUM_BLOCKS}"
                 debug_text_pos = [base_pos[0]-0.3, base_pos[1]+0.3, base_pos[2]+0.5]
                 if are_valid_coordinates(debug_text_pos):
                     p.addUserDebugText(imu_text, debug_text_pos, [1,1,1], 1.0, SIMULATION_TIMESTEP*2, replaceItemUniqueId=1)
                     ...
                     # Add text for Robot 1 state slightly lower
                     if state_text_r1:
                         p.addUserDebugText(state_text_r1, [debug_text_pos[0], debug_text_pos[1]-0.3, debug_text_pos[2]], [1,0.7,0], 1.0, SIMULATION_TIMESTEP*2, replaceItemUniqueId=4) # Orange for R1 state


            except p.error: pass
            except IndexError: pass # Handle if robots list isn't populated yet
            ...
            if current_time - last_print_time >= 1.0:
                 print("-" * 20)
                 for i, bot in enumerate(robots):
                     print(f"[INFO] Time: {current_time:.1f} Robot {i}: State={bot.get_state_name()} ({bot.current_state}) TargetBlock: {bot.target_block_id}")
                 print("-" * 20)
