...
PLACEMENT_OFFSET = 0.11 # Distance between placed blocks (X-axis)
...
# Define the block spawning area (min_x, max_x, min_y, max_y)
BLOCK_SPAWN_AREA = [-2.0, 2.0, 1.0, 3.0]
# Define default destination area (to ensure no overlap)
...
def load_robots(urdf_path, num_to_load):
    ...
    for i in range(num_to_load):
        ...
        start_pos = [start_x, 0, 0.1] # Start near origin Y=0
...
def load_blocks(block_urdf_path, num_to_load, spawn_area):
    """Loads blocks within a specified rectangular spawn_area [min_x, max_x, min_y, max_y]."""
    blocks = {}; block_states = {}
    ...
    for i in range(num_to_load):
        # <<< Generate coordinates within the defined spawn area >>>
        block_x = random.uniform(min_x, max_x)
        block_y = random.uniform(min_y, max_y)
        block_z = 0.01
...
class BuilderBot:
    def __init__(self, robot_instance_id, robot_id, joint_dict, block_viz_dict, shared_block_states, shared_data, target_vis_id, dest_vis_id):
        ...
        self.base_destination = list(DEFAULT_DESTINATION) # <<< Use default destination area
        ...
        # Sliders control base destination
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            ...
            # <<< Logic using shared_data for target assignment >>>
            my_target_id = -1
            rotate_only = False
            ...
            if self.robot_instance_id == 0 and not self.shared_data.get('block_targets_assigned', False):
                ...
                # print(f"[DEBUG] Robot 0: Performing target calculation...") # Reduce spam
                available_blocks = []
                r0_pos, _ = p.getBasePositionAndOrientation(self.robot_id) # Get R0's current position
                for block_id, status in self.block_states.items():
                    if status == 'AVAILABLE':
                        try:
                            pos_block, _ = p.getBasePositionAndOrientation(block_id)
                            if not are_valid_coordinates(pos_block): continue
                            dist_sq = (pos_block[0] - r0_pos[0])**2 + (pos_block[1] - r0_pos[1])**2
                ...
                self.shared_data['block_targets_assigned'] = True
                # print(f"[DEBUG] Robot 0: Calculation complete. Targets: R0={self.shared_data['target_for_robot_0']}, R1={self.shared_data['target_for_robot_1']}") # Reduce spam
            ...
            if self.shared_data.get('block_targets_assigned', False):
                ...
                if my_target_id != -1 and self.block_states.get(my_target_id) == 'AVAILABLE':
                    ...
                    try:
                        ...
                        if abs(angle_error) < FIND_BLOCK_ANGLE_THRESHOLD: print(f"[INFO] Robot {self.robot_instance_id}: FIND_BLOCK: Aligned with assigned block {self.target_block_id}. -> APPROACH_BLOCK"); self.current_state = STATE_APPROACH_BLOCK; self.angle_pid.reset(); # Flag reset moved to DOCKING
            ...
            # (Logic mostly unchanged, uses shared state check now)
            ...
            # (Logic unchanged, uses stability check)
             target_status = self.block_states.get(self.target_block_id, 'NOT_FOUND')
            ...
            # (Logic unchanged, but resets shared flag)
            target_status = self.block_states.get(self.target_block_id, 'NOT_FOUND')
            if self.target_block_id is None or target_status != 'AVAILABLE': self.current_state = STATE_FIND_BLOCK; return target
            ...
            # (Logic unchanged, uses shared count)
            ...
            current_placed_count = self.shared_data['placed_count']
            ...
            # (Logic unchanged - uses Y-axis alignment and stability check)
            ...
            # (Logic unchanged - uses yaw correction)
            target = self.current_placement_target; dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]; distance = math.hypot(dx, dy)
            ...
            # (Logic unchanged - updates shared state/count)
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    shared_data = {
        'placed_count': 0,
        'target_for_robot_0': -1, # Stores intended target ID for robot 0
        'target_for_robot_1': -1, # Stores intended target ID for robot 1
        'block_targets_assigned': False # Flag for coordination
    }
    ...
    for i in range(NUM_ROBOTS):
        bot = BuilderBot(
            ...
            block_viz_dict=block_visualizations, shared_block_states=block_states, # Pass shared dicts
        ...
        while p.isConnected():
            ...
            try:
                 ...
                 if auto_camera:
                     ...
                     if valid_active_bot_index < len(robots): # Ensure index is valid
            ...
            try:
                 # Display info for Robot 0
                 bot0 = robots[0]
                 base_pos0, base_orn0 = p.getBasePositionAndOrientation(bot0.robot_id)
                 roll0, pitch0, yaw0 = p.getEulerFromQuaternion(base_orn0)
                 imu0_text = f"R0 IMU: Y:{math.degrees(yaw0):.0f}"
                 state0_text = f"R0 St: {bot0.get_state_name()}"
                 target0_text= f"R0 Tgt: {bot0.target_block_id}"
                 ...
                 # Display info for Robot 1
                 if len(robots) > 1:
                     bot1 = robots[1]
                     base_pos1, base_orn1 = p.getBasePositionAndOrientation(bot1.robot_id)
                     roll1, pitch1, yaw1 = p.getEulerFromQuaternion(base_orn1)
                     imu1_text = f"R1 IMU: Y:{math.degrees(yaw1):.0f}"
                     state1_text = f"R1 St: {bot1.get_state_name()}"
                     target1_text= f"R1 Tgt: {bot1.target_block_id}"
                 ...
                 # Shared blocks placed count (display near Robot 0 for simplicity)
            ...
            if current_time - last_print_time >= 1.0:
                 ...
                 print(f"Shared Data: {shared_data}") # Print shared data periodically
                 print("-" * 20)
