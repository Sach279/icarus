...
RETREAT_SPEED = 1.5
RETREAT_DISTANCE = 0.18 # <<< Increased Retreat Distance slightly
...
ANGULAR_VELOCITY_STABILITY_THRESHOLD = 0.05 # rad/s
...
# <<< Corrected update_all_block_visualizations >>>
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            ...
            if closest_block_id != -1:
                ...
                if abs(angle_error) < FIND_BLOCK_ANGLE_THRESHOLD: print(f"[INFO] Robot {self.robot_instance_id}: FIND_BLOCK: Aligned with closest block {self.target_block_id}. -> APPROACH_BLOCK"); self.current_state = STATE_APPROACH_BLOCK; self.angle_pid.reset()
            ...
            elif available_blocks_exist: print(f"[WARN] Robot {self.robot_instance_id}: FIND_BLOCK: Available blocks exist, but failed to find closest/valid one. Rotating slowly."); left_velocity = -0.5; right_velocity = 0.5
            else: print(f"[INFO] Robot {self.robot_instance_id}: FIND_BLOCK: No available blocks found. -> ALL_DELIVERED."); self.current_state = STATE_ALL_DELIVERED
            ...
            if self.target_block_id is None or target_status != 'AVAILABLE': print(f"[WARN] Robot {self.robot_instance_id}: APPROACH_BLOCK: Invalid target block (ID: {self.target_block_id}, Status: '{target_status}'). -> FIND_BLOCK."); self.current_state = STATE_FIND_BLOCK; self.target_block_id = None; return target
            try:
             ...
             if self.target_block_id is None or self.block_states.get(self.target_block_id) != 'AVAILABLE': self.current_state = STATE_FIND_BLOCK; return target
             try:
                pos_block, orn_block = p.getBasePositionAndOrientation(self.target_block_id)
                if not are_valid_coordinates(pos_block): raise p.error("Invalid block coordinates")
                rot_mat = p.getMatrixFromQuaternion(orn_block); local_x = np.array([rot_mat[0], rot_mat[3]]); local_y = np.array([rot_mat[1], rot_mat[4]])
            ...
            if time.time() - self.final_drive_start_time > FINAL_DRIVE_TIMEOUT:
                ...
                if self.target_block_id is None or self.block_states.get(self.target_block_id) != 'AVAILABLE': self.current_state = STATE_FIND_BLOCK; self.final_drive_start_time = None; return target
                try:
            ...
            # <<< Use consistent Y-axis alignment >>>
            ...
            if not aligned: left_velocity = -pid_output * ROTATION_SPEED_SCALE; right_velocity = pid_output * ROTATION_SPEED_SCALE; self.reset_stability_timer()
                ...
                left_velocity = right_velocity = 0.0
                if self.check_stability(aligned, self.robot_id): # <<< Pass robot ID
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    shared_data = {'placed_count': 0} # Shared counter
    ...
    try:
        while p.isConnected():
            ...
            active_bot_index = 0 # Default camera focus
            ...
            for i, bot in enumerate(robots):
                try:
                    ...
                    # Update active bot index if this bot is not idle/finished
                    if bot.current_state != STATE_IDLE and bot.current_state != STATE_ALL_DELIVERED:
                        active_bot_index = i
                        camera_target_overall = current_camera_target # Camera follows this bot's target
            ...
            try:
                 update_all_block_visualizations(robots[0].block_viz_dict) # Use first bot's reference
                 if auto_camera:
                     active_bot_pos, _ = p.getBasePositionAndOrientation(robots[active_bot_index].robot_id)
                     update_camera_view(auto_camera, active_bot_pos, camera_target_overall)
                 if lidar_enabled:
                     for bot in robots: simulate_lidar(bot.robot_id)
            except p.error: pass
            except IndexError: pass # Handle if active_bot_index becomes invalid
            ...
            try:
                 ...
                 blocks_text = f"Placed: {bot0.shared_data['placed_count']}/{NUM_BLOCKS}" # Show shared count
            ...
            p.stepSimulation() # Step physics after all bots have calculated commands
