...
class PIDController:
    ...
    def compute(self, error, dt):
        # Clamp dt to avoid division by zero or huge derivatives if timestep is unstable
        dt = max(dt, 1e-6)
        self.error_integral += error * dt
        # Optional: Clamp integral term to prevent windup
        # MAX_INTEGRAL = 10.0 # Example limit
        # self.error_integral = max(min(self.error_integral, MAX_INTEGRAL), -MAX_INTEGRAL)
        derivative = (error - self.previous_error) / dt
        # Clamp derivative term to prevent spikes
        # MAX_DERIVATIVE = 100.0 # Example limit
...
def is_valid_coordinate(coord):
    """Checks if a single coordinate value is valid (not NaN or Inf)."""
    return not (math.isnan(coord) or math.isinf(coord))

def are_valid_coordinates(*coords):
    """Checks if all values in a list/tuple are valid coordinates."""
    return all(is_valid_coordinate(c) for point in coords for c in point)
...
def simulate_lidar(robot_id, num_rays=36, ray_length=5.0, fov_deg=140):
    try:
        ...
        for i in range(num_rays):
            ...
            if not (is_valid_coordinate(dx) and is_valid_coordinate(dy)):
                print(f"[WARN] LIDAR: Invalid dx/dy calculation for ray {i}, angle {angle:.2f}")
                continue # Skip this ray

            ray_from = sensor_pos
            ray_to = [sensor_pos[0]+dx, sensor_pos[1]+dy, sensor_pos[2]]

            # Check final coordinates before adding to list
            if not are_valid_coordinates(ray_from, ray_to):
                 print(f"[WARN] LIDAR: Invalid ray coordinates for ray {i}. From={ray_from}, To={ray_to}")
                 continue # Skip this ray

            ray_from_list.append(ray_from)
            ray_to_list.append(ray_to)

        if not ray_from_list: return # Skip batch if all rays were invalid
        ...
        for i, res in enumerate(results):
            ...
            if are_valid_coordinates(ray_from_list[i], hit_pos):
                try:
                    ...
                    # This is where the "User debug draw failed" might explicitly originate
                    print(f"[WARN] LIDAR p.addUserDebugLine failed: {e}. From={ray_from_list[i]}, To={hit_pos}")
            # else:
                # print(f"[WARN] LIDAR: Skipping draw for invalid coordinates. From={ray_from_list[i]}, To={hit_pos}")

    except p.error as e:
        # Catch errors getting robot pose etc.
        print(f"[WARN] PyBullet error during Lidar simulation: {e}")
    except Exception as e:
        # Catch other potential math errors etc.
        print(f"[ERROR] Unexpected error in simulate_lidar: {e}")
        traceback.print_exc()
...
def set_motor_speeds(robot_id, joint_dict, left_velocity, right_velocity):
    # Add basic validation for velocity commands
    ...
    except p.error: pass # Ignore if robot removed etc.
...
def update_all_block_visualizations(block_viz_dict):
    ...
    for block_id, sphere_id in block_viz_dict.items():
        try:
            ...
            if not are_valid_coordinates(pos_block):
                # print(f"[WARN] Block {block_id} has invalid position, skipping visualization.")
                continue
            ...
            # Define end points and check coordinates before drawing each line
            ...
            if are_valid_coordinates(pos_block, end_x):
                ...
                except p.error as e: print(f"[WARN] Debug draw failed for block {block_id} X-axis: {e}")
            # else: print(f"[WARN] Invalid coords for block {block_id} X-axis")
            ...
            if are_valid_coordinates(pos_block, end_y):
                 ...
                 except p.error as e: print(f"[WARN] Debug draw failed for block {block_id} Y-axis: {e}")
            # else: print(f"[WARN] Invalid coords for block {block_id} Y-axis")
            ...
            if are_valid_coordinates(pos_block, end_z):
                 ...
                 except p.error as e: print(f"[WARN] Debug draw failed for block {block_id} Z-axis: {e}")
            # else: print(f"[WARN] Invalid coords for block {block_id} Z-axis")

        except p.error: pass # Ignore if block ID is invalid
        ...
        if self.current_state == STATE_IDLE:
            ...
            for block_id, status in self.block_states.items():
                ...
                try:
                    ...
                    if not are_valid_coordinates(pos_block): continue # Skip if invalid position
            ...
            if self.target_block_id is None or self.block_states.get(self.target_block_id) != 'AVAILABLE': print("[WARN] APPROACH_BLOCK: Invalid target block. Returning to FIND_BLOCK."); self.current_state = STATE_FIND_BLOCK; self.target_block_id = None; return target
            try:
                pos_block, orn_block = p.getBasePositionAndOrientation(self.target_block_id)
                if not are_valid_coordinates(pos_block): raise p.error("Invalid block coordinates") # Raise error if invalid
                ...
                else: print("[INFO] APPROACH_BLOCK: Reached offset point. Transition to ALIGN_BLOCK"); self.current_state = STATE_ALIGN_BLOCK; self.angle_pid.reset(); self.reset_stability_timer(); p.resetBasePositionAndOrientation(self.target_vis_id, [0,0,-1], [0,0,0,1]); left_velocity = right_velocity = 0.0
            ...
            target = self.current_placement_target; dx = target[0] - base_pos[0]; dy = target[1] - base_pos[1]
            ...
            # print("[INFO] ALL_DELIVERED: All blocks placed. Task complete.") # Reduce printing
            left_velocity = right_velocity = 0.0
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    try:
        while p.isConnected():
            ...
            except p.error as e: pass # Ignore text errors
