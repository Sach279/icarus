...
# --- PID Parameters ---
# Slightly more aggressive Kp, slightly more damping Kd
KP = 2.2; KI = 0.05; KD = 0.03 # <<< Adjusted Gains (Example - Requires Tuning!)

# --- Thresholds and Distances ---
FIND_BLOCK_ANGLE_THRESHOLD = math.radians(20.0) # <<< Relaxed Initial Find Threshold
APPROACH_BLOCK_STOP_DISTANCE = 0.05
ALIGNMENT_STABILITY_THRESHOLD = math.radians(1.0) # Keep this tight for final alignment
...
class BuilderBot:
    ...
    def run_step(self, all_robots):
        # ... (Get pose, initialize variables - unchanged) ...
        ...
        # ... (Rest of the states STATE_APPROACH_BLOCK through STATE_ALL_DELIVERED remain unchanged from the previous version) ...
            ...
            desired_angle = math.atan2(dy, dx); angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
            pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP)
            distance_to_lock_radius = distance - DEST_LOCK_RADIUS
            ...
            # <<< Use the same "point away" logic as ALIGN_BLOCK for consistency >>>
            dx = target[0] - base_pos[0]
            dy = target[1] - base_pos[1]
            # If distance is near zero, desired angle is ill-defined, use previous yaw
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    try:
        while p.isConnected():
            ...
            p.stepSimulation()
            # If using RealTimeSimulation(1), add a sleep to control speed
            time.sleep(SIMULATION_TIMESTEP) # Maybe slightly longer for visibility? e.g., 1/60.
        ...
        # Ensure Tkinter window is closed if simulation ends abruptly
        try:
            if grid_window_root: grid_window_root.destroy()
