...
def pickStrictSideAngle(dx, dy):
    ...
    based on which absolute coordinate is larger, so we approach from a side face.
    """
    if abs(dx) >= abs(dy):
...
def load_and_control_urdf(urdf_path):
    ...
    # threshold_stage1 = ±90°, threshold_stage2 = ±40°
    ...
    # Create a red transparent cube for the destination.
    dest_shape_id = p.createVisualShape(
        shapeType=p.GEOM_BOX,
        halfExtents=[0.2,0.2,0.2],
        rgbaColor=[1,0,0,0.5]
    )
    dest_body_id = p.createMultiBody(
        baseMass=0,
        baseVisualShapeIndex=dest_shape_id,
        ...
        while True:
            ...
            match state:
                case 0:  # APPROACH_BLOCK
                    ...
                    # If the angle error is bigger than threshold_stage1 => rover "loses sight" => stay in state 0.
                    # Actually, we are already in state 0, so we just do normal approach.
                    if abs(angle_error) > threshold_stage1:
                        print("[DEBUG] Lost block from sight => remain in state 0, rotating in place.")
                        # rotate in place to reacquire
                        left_velocity = -Kp * angle_error
                        right_velocity = Kp * angle_error
                    else:
                        # normal approach
                        if distance > final_approach_distance + predock_offset:
                            left_velocity = forward_speed - (Kp * angle_error)
                            right_velocity = forward_speed + (Kp * angle_error)
                        else:
                ...
                case 1:  # FINAL_ALIGN_BLOCK
                    ...
                    # If angle error is bigger than threshold_stage1 => go back to state 0.
                    # That means we "lost sight" of the block.
                    ...
                    if abs(angle_error_check) > threshold_stage1:
                        print("[DEBUG] Lost block from sight => revert to state 0.")
                        state = 0
                        left_velocity = 0.0
                        right_velocity = 0.0
                        # jump out of the match-case
                        break

                    if pre_dock_target_block is None:
                        pre_dock_target_block = [
                            pos_block[0] - predock_offset * math.cos(desired_angle_for_check),
                            pos_block[1] - predock_offset * math.sin(desired_angle_for_check)
                        ]
                    target = pre_dock_target_block
                    ...
                    # If still far or misaligned => keep moving slowly
                    if distance > final_approach_distance or abs(angle_error) > stop_alignment_threshold:
                        ...
                        if ord('d') in keys and keys[ord('d')] & p.KEY_WAS_TRIGGERED:
                            ...
                            dock_constraint = p.createConstraint(
                                robot_id, -1,
                                block_body_id, -1,
                                p.JOINT_FIXED, [0,0,0],
                                relPos, [0,0,0]
            ...
            if frame_count % 120 == 0:
                ...
                if 'distance' not in locals():
                    distance = 0.0
                print(f"[DEBUG] Frame {frame_count}: Dist={distance:.2f}, AngleErr={math.degrees(angle_error):.1f}, State={state}")
