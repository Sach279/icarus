...
APPROACH_SPEED_CAP = 0.7 * FORWARD_SPEED

# --- PID Parameters ---
KP = 1.8; KI = 0.05; KD = 0.02

# --- Thresholds and Distances ---
FIND_BLOCK_ANGLE_THRESHOLD = math.radians(15.0)
APPROACH_BLOCK_STOP_DISTANCE = 0.05
ALIGNMENT_STABILITY_THRESHOLD = math.radians(1.0)
ANGULAR_VELOCITY_STABILITY_THRESHOLD = 0.02
...
# --- Grid Map Parameters ---
GRID_RESOLUTION = 0.1
GRID_MIN_X = -3.0; GRID_MAX_X = 3.0
GRID_MIN_Y = -3.0; GRID_MAX_Y = 4.0
GRID_COLS = math.ceil((GRID_MAX_X - GRID_MIN_X) / GRID_RESOLUTION)
GRID_ROWS = math.ceil((GRID_MAX_Y - GRID_MIN_Y) / GRID_RESOLUTION)
GRID_FREE = 0; GRID_BLOCK_PLACED = 1; GRID_OBSTACLE = 2

# --- Debug Drawing Lifetime (in seconds) ---
DEBUG_DRAW_LIFETIME = 0.25
TRAIL_UPDATE_INTERVAL = 1.0 # <<< Interval to update robot trails
TRAIL_POINT_LIFETIME = 30.0 # <<< How long each trail segment lasts
    ...
    try:
        ...
        for i, res in enumerate(results):
            ...
            if are_valid_coordinates(ray_from_list[i], hit_pos):
                try: p.addUserDebugLine(ray_from_list[i], hit_pos, lineColorRGB=color, lineWidth=1, lifeTime=DEBUG_DRAW_LIFETIME)
                except p.error: pass
    except p.error: pass
    ...
    for block_id, sphere_id in block_viz_dict.items():
        try:
            ...
            if are_valid_coordinates(pos_block, end_x):
                try: p.addUserDebugLine(pos_block, end_x, [1,0,0], lifeTime=DEBUG_DRAW_LIFETIME)
                except p.error: pass
            ...
            if are_valid_coordinates(pos_block, end_y):
                 try: p.addUserDebugLine(pos_block, end_y, [0,1,0], lifeTime=DEBUG_DRAW_LIFETIME)
                 except p.error: pass
            ...
            if are_valid_coordinates(pos_block, end_z):
                 try: p.addUserDebugLine(pos_block, end_z, [0,0,1], lifeTime=DEBUG_DRAW_LIFETIME)
                 except p.error: pass
        except p.error: pass
...
class BuilderBot:
    def __init__(self, robot_instance_id, robot_id, joint_dict, block_viz_dict, shared_block_states, shared_data, target_vis_id, dest_vis_id):
        ...
        self.trail_points = [] # <<< Store trail points for this robot
        self.trail_line_color = [random.random()*0.8, random.random()*0.8, random.random()*0.8] # Unique color for each robot's trail
        self.last_trail_update_time = 0.0
    ...
    def update_target_line(self, base_pos):
        ...
        if self.target_block_id is not None and self.current_state >= STATE_APPROACH_BLOCK and self.current_state <= STATE_FINAL_DRIVE_BLOCK:
            try:
                target_pos_block, _ = p.getBasePositionAndOrientation(self.target_block_id)
                if are_valid_coordinates(base_pos, target_pos_block):
                    self.target_debug_line = p.addUserDebugLine(base_pos, [target_pos_block[0], target_pos_block[1], base_pos[2]], [0,1,0], 2, DEBUG_DRAW_LIFETIME)
            except p.error: self.target_debug_line = None

    def update_robot_trail(self, current_time, base_pos):
        """Adds a point to the robot's trail if enough time has passed."""
        if current_time - self.last_trail_update_time >= TRAIL_UPDATE_INTERVAL:
            # Add current position to the trail
            if are_valid_coordinates(base_pos):
                # Keep a limited number of trail points if desired, for now, keeps all
                # if len(self.trail_points) > MAX_TRAIL_POINTS:
                #     self.trail_points.pop(0)
                self.trail_points.append(list(base_pos)) # Store a copy
            ...
            if len(self.trail_points) > 1:
                for i in range(len(self.trail_points) - 1):
                    ...
                    try:
                        # Draw with a lifetime so they eventually disappear
                        p.addUserDebugLine(p1, p2, self.trail_line_color, lineWidth=1.5, lifeTime=TRAIL_POINT_LIFETIME)
                    except p.error: pass # Ignore drawing errors if points become invalid


    def run_step(self, all_robots, current_sim_time): # <<< Pass current_sim_time
        ...
        self.update_robot_trail(current_sim_time, base_pos) # <<< Update trail
        ...
        if self.current_state == STATE_IDLE:
            ...
            if self.robot_instance_id == 0 and not self.shared_data.get('block_targets_assigned', False):
                ...
                available_blocks.sort(); self.shared_data['target_for_robot_0'] = available_blocks[0][1] if len(available_blocks) > 0 else -1; self.shared_data['target_for_robot_1'] = available_blocks[1][1] if len(available_blocks) > 1 else -1; self.shared_data['block_targets_assigned'] = True;
            if self.shared_data.get('block_targets_assigned', False):
                ...
                if my_target_id != -1 and self.block_states.get(my_target_id) == 'AVAILABLE':
                    ...
                    try:
                        ...
                        else: pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP); left_velocity = -pid_output * ROTATION_SPEED_SCALE; right_velocity = pid_output * ROTATION_SPEED_SCALE; rotate_only = True
            ...
            desired_angle = math.atan2(dy, dx); angle_error = (desired_angle - rover_yaw + math.pi) % (2*math.pi) - math.pi
            pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP)
            distance_to_lock_edge = distance - DEST_LOCK_RADIUS
            current_locker = self.shared_data['destination_locked_by']
            if distance <= DEST_LOCK_RADIUS and current_locker == -1:
            ...
            elif distance <= DEST_LOCK_RADIUS and current_locker == self.robot_instance_id:
                 print(f"[INFO] Robot {self.robot_instance_id}: APPROACH_DEST: Inside lock radius (have lock). -> ALIGN_DEST.")
                 self.current_state = STATE_ALIGN_DEST; self.angle_pid.reset(); self.reset_stability_timer(); left_velocity = right_velocity = 0.0
            elif current_locker != -1 and current_locker != self.robot_instance_id:
                 if distance_to_lock_edge <= 0.5:
                      print(f"[INFO] Robot {self.robot_instance_id}: APPROACH_DEST: Dest locked by Robot {current_locker}. Stopping outside lock radius.")
                      left_velocity = right_velocity = 0.0
                      self.current_state = STATE_WAITING_FOR_DEST
                 else:
                      current_speed = min(FORWARD_SPEED, APPROACH_SPEED_CAP)
                      left_velocity = current_speed - pid_output * 0.5
                      right_velocity = current_speed + pid_output * 0.5
            else:
                 current_speed = min(FORWARD_SPEED, APPROACH_SPEED_CAP)
                 left_velocity = current_speed - pid_output * 0.5
                 right_velocity = current_speed + pid_output * 0.5
            ...
            if math.hypot(dx, dy) < 0.01: desired_angle = rover_yaw
            else: desired_angle = math.atan2(dy, dx) + math.pi
...
def run_simulation(urdf_path, block_urdf_path):
    # (Initialize simulation, load assets - unchanged)
    ...
    dest_body_id = create_destination_visual(); target_vis_id = create_target_visual(); lidar_enabled = False; auto_camera = True
    ...
    try:
        while p.isConnected():
                ...
                if bot.dock_constraint is not None:
                    try:
                        ...
                        if are_valid_coordinates(bot_base_pos, dest_pos):
                            ...
                            transport_line_ids[i] = p.addUserDebugLine(bot_base_pos, [dest_pos[0], dest_pos[1], bot_base_pos[2]], [0, 1, 1], 2, DEBUG_DRAW_LIFETIME)
            ...
            try:
                 ...
                 if are_valid_coordinates(debug_pos0):
                     p.addUserDebugText(imu0_text, debug_pos0, [1,1,1], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=1)
                     p.addUserDebugText(state0_text, [debug_pos0[0], debug_pos0[1]-0.1, debug_pos0[2]], [1,1,0], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=2)
                     p.addUserDebugText(target0_text, [debug_pos0[0], debug_pos0[1]-0.2, debug_pos0[2]], [1,0.7,0], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=4)
                 if len(robots) > 1:
                     ...
                     if are_valid_coordinates(debug_pos1):
                        p.addUserDebugText(imu1_text, debug_pos1, [1,1,1], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=5)
                        p.addUserDebugText(state1_text, [debug_pos1[0], debug_pos1[1]-0.1, debug_pos1[2]], [1,1,0], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=6)
                        p.addUserDebugText(target1_text, [debug_pos1[0], debug_pos1[1]-0.2, debug_pos1[2]], [1,0.7,0], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=7)
                 blocks_text = f"Placed: {shared_data['placed_count']}/{NUM_BLOCKS}"
                 p.addUserDebugText(blocks_text, [debug_pos0[0], debug_pos0[1]-0.3, debug_pos0[2]], [0,1,1], 1.0, DEBUG_DRAW_LIFETIME, replaceItemUniqueId=3)
            except p.error: pass
            except IndexError: pass

            p.stepSimulation()
            time.sleep(SIMULATION_TIMESTEP) # Match this to p.setRealTimeSimulation(0) if you change it
        ...
        try:
            if 'grid_window_root' in locals() and grid_window_root.winfo_exists(): grid_window_root.destroy()
