...
def load_and_control_urdf(urdf_path):
    ...
    try:
        block_body_id = p.loadURDF(block_urdf_path, basePosition=[block_x, block_y, block_z], useFixedBase=False)
        print(f"[DEBUG] Block loaded at random position: ({block_x:.2f}, {block_y:.2f}, {block_z})")
    except Exception as e:
