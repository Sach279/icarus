...
# <<< Updated Parameters >>>
ALIGNMENT_STABILITY_THRESHOLD = math.radians(2.0) # Relaxed slightly: 2 degrees
FINAL_APPROACH_DISTANCE = 0.12 # Already 0.12, no change needed but noted
# <<< End of Updated Parameters >>>
ANGULAR_VELOCITY_STABILITY_THRESHOLD = 0.05 # rad/s
STABILITY_DURATION = 0.75
DOCKING_DISTANCE = 0.11
FINAL_DRIVE_TIMEOUT = 20.0

# --- Simulation Parameters ---
SIMULATION_TIMESTEP = 1/240.0
# <<< Updated Parameter >>>
NUM_BLOCKS = 10
# <<< End of Updated Parameter >>>
NUM_ROBOTS = 2
...
class BuilderBot:
    def __init__(self, robot_instance_id, robot_id, joint_dict, block_viz_dict, shared_block_states, shared_data, target_vis_id, dest_vis_id):
        ...
        self.base_destination = list(DEFAULT_DESTINATION) # Use default initially
        self.current_placement_target = [0.0, 0.0]
        # Removed self.placed_block_count
    ...
    def run_step(self):
        ...
        if self.current_state == STATE_IDLE:
            if self.stability_timer is None: self.stability_timer = time.time()
            if time.time() - self.stability_timer >= 1.0 + random.uniform(0, 0.5*self.robot_instance_id): # Stagger start slightly more
                self.current_state = STATE_FIND_BLOCK; self.reset_stability_timer(); self.angle_pid.reset(); print(f"[INFO] Robot {self.robot_instance_id}: IDLE complete -> FIND_BLOCK")
            ...
            # --- Only Robot 0 Calculates Targets, Once per Cycle ---
            if self.robot_instance_id == 0 and not self.shared_data.get('block_targets_assigned', False):
                ...
                print(f"[DEBUG] Robot 0: Calculation complete. Targets: R0={self.shared_data['target_for_robot_0']}, R1={self.shared_data['target_for_robot_1']}")
            ...
            placement_index = self.shared_data['next_placement_slot_index'] # Read the *next* available slot index

            target_x = self.base_destination[0] + placement_index * PLACEMENT_OFFSET
            target_y = self.base_destination[1]
            self.current_placement_target = [target_x, target_y]; target = self.current_placement_target

            # <<< IMMEDIATELY Increment the shared index to claim this slot >>>
            ...
            # (Logic unchanged - drives to self.current_placement_target)
            ...
            # <<< Only increments shared placed_count >>>
            if self.dock_constraint is not None:
                ...
                if self.target_block_id is not None:
                    ...
                    self.shared_data['placed_count'] += 1 # <<< Increment shared SUCCESSFUL placement count
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    # <<< Initialize Shared Data with new index >>>
    shared_data = {
        'placed_count': 0,                # Tracks successful placements
        'next_placement_slot_index': 0, # Tracks the next target slot to assign
        ...
        while p.isConnected():
            ...
            try:
                 ...
                 if are_valid_coordinates(debug_pos0):
                     ...
                     p.addUserDebugText(target0_text, [debug_pos0[0], debug_pos0[1]-0.2, debug_pos0[2]], [1,0.7,0], 1.0, SIMULATION_TIMESTEP*2, replaceItemUniqueId=4) # Target text ID 4
                 if len(robots) > 1:
                     ...
                     debug_pos1 = [base_pos1[0]-0.3, base_pos1[1]+0.4, base_pos1[2]+0.5] # Position relative to bot 1
                     if are_valid_coordinates(debug_pos1):
                        ...
                        p.addUserDebugText(target1_text, [debug_pos1[0], debug_pos1[1]-0.2, debug_pos1[2]], [1,0.7,0], 1.0, SIMULATION_TIMESTEP*2, replaceItemUniqueId=7) # Target text ID 7
                 blocks_text = f"Placed: {shared_data['placed_count']}/{NUM_BLOCKS}" # Use shared count
                 p.addUserDebugText(blocks_text, [debug_pos0[0], debug_pos0[1]-0.3, debug_pos0[2]], [0,1,1], 1.0, SIMULATION_TIMESTEP*2, replaceItemUniqueId=3) # Blocks text ID 3
            except p.error: pass
            except IndexError: pass
