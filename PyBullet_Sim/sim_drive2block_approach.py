...
def load_and_control_urdf(urdf_path):
    ...
    # Two-stage alignment thresholds:
    threshold_stage1 = math.radians(90)  # Stage 1: if error > 90° -> rotate on the spot.
    threshold_stage2 = math.radians(40)  # Stage 2: between 90° and 40° -> transitional drive.
    # Final approach: when close to target, force perpendicular alignment.
    ...
    # Load the target from CubeStructure.urdf using its absolute path.
    target_urdf_path = r"C:\Users\akshi\Documents\Building Block\Models\CubeStructure.urdf"
    try:
        target_body_id = p.loadURDF(target_urdf_path, basePosition=[2.0, 2.0, 0.2], useFixedBase=True)
        print("[DEBUG] CubeStructure.urdf loaded as target.")
    except Exception as e:
    ...
    try:
        while True:
            ...
            # Update the target's position using the URDF.
            ...
            # Compute vector from rover to target and distance.
            ...
            if distance < final_approach_distance:
                # For final approach, override desired angle to be perpendicular to the closest face.
                # For an axis-aligned cube, we choose the cardinal direction corresponding to the dominant component.
                if abs(dx) > abs(dy):
                    if dx > 0:
                        discrete_desired_angle = 0.0         # Approach from -X face: rover heading 0° (pointing +X)
                    else:
                        discrete_desired_angle = math.pi       # Approach from +X face: rover heading 180° (pointing -X)
                else:
                    if dy > 0:
                        discrete_desired_angle = math.pi/2     # Approach from -Y face: rover heading 90° (pointing +Y)
                    else:
                        discrete_desired_angle = -math.pi/2    # Approach from +Y face: rover heading -90° (pointing -Y)
                desired_angle = discrete_desired_angle
                if frame_count % 120 == 0:
                    print(f"[DEBUG] Final approach: Overriding desired angle to {math.degrees(desired_angle):.0f}° based on relative position (dx={dx:.2f}, dy={dy:.2f}).")
            else:
                # Otherwise, compute desired angle normally.
