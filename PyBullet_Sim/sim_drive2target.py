...
def load_and_control_urdf(urdf_path):
    ...
    forward_speed = 10.0  # Base forward speed.
    Kp = 2.0             # Proportional gain for turning.
    # Threshold (in radians) to decide if the target is "centered" (20° threshold).
    alignment_threshold = math.radians(90)
        ...
        while True:
            ...
            if distance < 0.2:
                ...
                if abs(angle_error) > alignment_threshold:
                    ...
                    if frame_count % 120 == 0:
                        print("[DEBUG] Rotating on the spot to align with target.")
                else:
                    # Drive forward.
                    desired_angle = math.atan2(dy, dx)
                    angle_error = desired_angle - rover_yaw
                    ...
                    if frame_count % 120 == 0:
                        print("[DEBUG] Target aligned. Driving forward.")

            # Apply computed velocities to the wheel joints.
            ...
            # In Free Camera mode, the camera is not automatically updated.
