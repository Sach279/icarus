...
def load_and_control_urdf(urdf_path):
    ...
    # Spawn the block at a random position between 1 and 3 units from the origin.
    r = random.uniform(1, 3)
    angle = random.uniform(0, 2 * math.pi)
    block_x = r * math.cos(angle)
    block_y = r * math.sin(angle)
    ...
    try:
        ...
        print(
            f"[DEBUG] Block loaded at random position: ({block_x:.2f}, {block_y:.2f}, {block_z}) with rotation {math.degrees(random_z_rotation):.1f}°")
    except Exception as e:
        ...
        while True:
            ...
            match state:
                ...
                case 1:  # FINAL_ALIGN_BLOCK
                    # Get current block and rover positions/orientations
                    pos_block, orn_block = p.getBasePositionAndOrientation(block_body_id)
                    dx = pos_block[0] - base_pos[0]
                    dy = pos_block[1] - base_pos[1]
                    distance = math.hypot(dx, dy)

                    # Compute a discrete angle for checking
                    desired_angle_for_check = pickStrictSideAngle(dx, dy)
                    angle_error_check = desired_angle_for_check - rover_yaw
                    angle_error_check = (angle_error_check + math.pi) % (2 * math.pi) - math.pi

                    # Debug: Print basic block-to-rover info
                    print(
                        f"[DEBUG] FINAL_ALIGN_BLOCK: pos_block=({pos_block[0]:.2f}, {pos_block[1]:.2f}), dx={dx:.2f}, dy={dy:.2f}, distance={distance:.2f}")
                    print(
                        f"[DEBUG] FINAL_ALIGN_BLOCK: desired_angle_for_check={math.degrees(desired_angle_for_check):.1f}°, rover_yaw={math.degrees(rover_yaw):.1f}°, angle_error_check={math.degrees(angle_error_check):.1f}°")

                    # If the block is "lost" (angle error > 90°), revert to state 0.
                    if abs(angle_error_check) > threshold_stage1:
                        print("[DEBUG] Lost block from sight => reverting to state 0.")
                        state = 0
                        left_velocity = 0.0
                        right_velocity = 0.0
                        break

                    # For relative orientation, compute block's local x and y axes.
                    rot_mat = p.getMatrixFromQuaternion(orn_block)
                    local_x = np.array([rot_mat[0], rot_mat[3], rot_mat[6]])
                    local_y = np.array([rot_mat[1], rot_mat[4], rot_mat[7]])

                    # Compute vector from block to rover (only horizontal component)
                    vec = np.array(base_pos[:2]) - np.array(pos_block[:2])
                    norm = np.linalg.norm(vec)
                    if norm > 1e-3:
                        vec_norm = vec / norm
                    else:
                        vec_norm = np.array([1, 0])

                    dot_x = np.dot(vec_norm, local_x[:2])
                    dot_y = np.dot(vec_norm, local_y[:2])

                    # Choose the docking face: the local axis with greater absolute dot product.
                    ...
                    # Compute the angle error relative to desired docking face.
                    angle_error = desired_angle - rover_yaw
                    angle_error = (angle_error + math.pi) % (2 * math.pi) - math.pi

                    # Debug: Print the local axes and dot products.
                    print(
                        f"[DEBUG] FINAL_ALIGN_BLOCK: local_x=({local_x[0]:.2f}, {local_x[1]:.2f}), local_y=({local_y[0]:.2f}, {local_y[1]:.2f})")
                    print(f"[DEBUG] FINAL_ALIGN_BLOCK: dot_x={dot_x:.2f}, dot_y={dot_y:.2f}")
                    print(
                        f"[DEBUG] FINAL_ALIGN_BLOCK: chosen_axis=({chosen_axis[0]:.2f}, {chosen_axis[1]:.2f}), desired_angle={math.degrees(desired_angle):.1f}°, angle_error={math.degrees(angle_error):.1f}°")

                    # Use PID to control rotation until aligned.
                    error = angle_error
                    error_integral += error * dt
                    error_derivative = (error - previous_error) / dt
                    pid_output = Kp * error + Ki * error_integral + Kd * error_derivative
                    ...
                    if abs(angle_error) > stop_alignment_threshold:
                        ...
                        right_velocity = 0.0
                        stage = 4
                        print("[DEBUG] FINAL_ALIGN_BLOCK: 90° turn complete and stopped.")

                    if stage == 4:
                        print("[DEBUG] FINAL_ALIGN_BLOCK: Driving forward after alignment.")
                    ...
                    if stage == 5:
                        print("[DEBUG] FINAL_ALIGN_BLOCK: Perfectly aligned and stopped.")
                ...
                case 3:  # FINAL_ALIGN_DEST: (Can implement similar logic for destination alignment)
                    dx_slider = p.readUserDebugParameter(dest_x_slider)
                    dy_slider = p.readUserDebugParameter(dest_y_slider)
                    target = [dx_slider, dy_slider]
