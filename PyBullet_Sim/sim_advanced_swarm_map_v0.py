...
# --- Grid Map Parameters ---
GRID_RESOLUTION = 0.1 # Meters per grid cell <<< Chosen Parameter
GRID_MIN_X = -3.0     # <<< Chosen Boundary
GRID_MAX_X = 3.0      # <<< Chosen Boundary
GRID_MIN_Y = -3.0     # <<< Chosen Boundary
GRID_MAX_Y = 4.0      # <<< Chosen Boundary
# Calculate grid dimensions based on resolution and bounds
GRID_COLS = math.ceil((GRID_MAX_X - GRID_MIN_X) / GRID_RESOLUTION)
GRID_ROWS = math.ceil((GRID_MAX_Y - GRID_MIN_Y) / GRID_RESOLUTION)

# --- Grid Cell States (for A* map) ---
GRID_FREE = 0
GRID_BLOCK_PLACED = 1 # A cell occupied by a successfully placed block
GRID_BLOCK_TARGET = -1 # A cell representing a placement target for A* (optional representation)
GRID_OBSTACLE = 2 # Generic obstacle (could be static or dynamic)
...
def world_to_grid(x, y):
    """Converts world coordinates (x, y) to grid coordinates (row, col)."""
    if not (is_valid_coordinate(x) and is_valid_coordinate(y)):
        return -1, -1 # Indicate invalid grid cell

    col = int((x - GRID_MIN_X) / GRID_RESOLUTION)
    row = int((y - GRID_MAX_Y) / -GRID_RESOLUTION) # Invert Y for display (0 at top)
...
def grid_to_world_center(row, col):
    """Converts grid coordinates (row, col) to world coordinates of the cell center."""
    if not (0 <= row < GRID_ROWS and 0 <= col < GRID_COLS):
        # Return center of the grid or a default invalid coordinate
        return (GRID_MIN_X + GRID_MAX_X) / 2.0, (GRID_MIN_Y + GRID_MAX_Y) / 2.0

    x = GRID_MIN_X + (col + 0.5) * GRID_RESOLUTION
    y = GRID_MAX_Y - (row + 0.5) * GRID_RESOLUTION # Invert Y calculation

    return x, y

def create_grid_map():
    """Initializes a new grid map filled with FREE cells."""
    return np.full((GRID_ROWS, GRID_COLS), GRID_FREE, dtype=int)

def update_grid_map(grid_map, robots, blocks, placed_blocks):
    """Updates the grid map with obstacle information (placed blocks, other robots)."""
    # Reset dynamic obstacles (robots)
    # We don't need to reset placed blocks if we only add them incrementally
    # grid_map.fill(GRID_FREE) # <-- Only if you were resetting everything each time

    # Mark placed blocks (these are more static)
    for block_id in placed_blocks:
         try:
             pos, _ = p.getBasePositionAndOrientation(block_id)
             # Mark the cell and potentially neighbors as occupied
             row, col = world_to_grid(pos[0], pos[1])
             if row != -1 and col != -1:
                 # Simple padding: mark the block's cell and maybe immediate neighbors
                 # More sophisticated padding would consider robot radius vs cell size
                 padding = 1 # Number of cells to pad around obstacles
                 for r in range(max(0, row - padding), min(GRID_ROWS, row + padding + 1)):
                     for c in range(max(0, col - padding), min(GRID_COLS, col + padding + 1)):
                         grid_map[r, c] = GRID_BLOCK_PLACED # Use a specific value for placed blocks
         except p.error: continue # Ignore if block is gone


    # Mark other robots (dynamic obstacles)
    for robot in robots:
         try:
             pos, _ = p.getBasePositionAndOrientation(robot.robot_id)
             row, col = world_to_grid(pos[0], pos[1])
             if row != -1 and col != -1:
                 # Mark the robot's cell and maybe neighbors as occupied
                 padding = 1 # Pad robots too
                 for r in range(max(0, row - padding), min(GRID_ROWS, row + padding + 1)):
                     for c in range(max(0, col - padding), min(GRID_COLS, col + padding + 1)):
                         # Only mark as OBSTACLE if not already a PLACED block
                         if grid_map[r, c] != GRID_BLOCK_PLACED:
                              grid_map[r, c] = GRID_OBSTACLE
         except p.error: continue # Ignore if robot is gone

    # print("Grid Map Updated") # Debug print
    return grid_map
...
def create_grid_window(grid_map):
    """Creates and returns the Tkinter root and canvas for grid visualization."""
    root = tk.Tk()
    root.title("Grid Map Visualization")

    # Scale factor: pixels per grid cell
    cell_pixel_size = min(CANVAS_WIDTH / GRID_COLS, CANVAS_HEIGHT / GRID_ROWS)

    # Calculate canvas dimensions to match the grid aspect ratio while fitting window
    canvas_w = int(GRID_COLS * cell_pixel_size)
    canvas_h = int(GRID_ROWS * cell_pixel_size)
    ...
    root.grid_data = {
        ...
        'offset_x': (CANVAS_WIDTH - canvas_w) / 2, # Center grid on canvas if not filling
        'offset_y': (CANVAS_HEIGHT - canvas_h) / 2,
        'grid_map': grid_map # Store reference to the shared map
    }

    draw_grid_map(root.grid_data) # Initial draw

    # Use update_idletasks for integration with PyBullet loop
    # root.update_idletasks() # Done in main loop

    return root

def draw_grid_map(grid_data):
    """Draws/updates the grid on the Tkinter canvas."""
    canvas = grid_data['canvas']
    grid_map = grid_data['grid_map']
    cell_pixel_size = grid_data['cell_pixel_size']
    ...
    for r in range(GRID_ROWS):
        for c in range(GRID_COLS):
            ...
            x1 = c * cell_pixel_size + offset_x
            y1 = r * cell_pixel_size + offset_y
            x2 = x1 + cell_pixel_size
            y2 = y1 + cell_pixel_size

            color = 'white' # GRID_FREE
            if cell_value == GRID_BLOCK_PLACED: color = 'gray' # Placed Block (solid)
            elif cell_value == GRID_OBSTACLE: color = 'red' # Dynamic Obstacle (robot)
            elif cell_value == GRID_BLOCK_TARGET: color = 'blue' # Placement Target (can draw separately)
            # Add colors for other states if needed

            canvas.create_rectangle(x1, y1, x2, y2, fill=color, outline='lightgray')

    # Optional: Draw robot positions as circles, block targets as small squares
    # This requires getting robot/block positions and converting to canvas coordinates
    # For now, just drawing the grid cells based on the map


# --- PyBullet Helper Functions (Rest) ---
...
class BuilderBot:
    def __init__(self, robot_instance_id, robot_id, joint_dict, block_viz_dict, shared_block_states, shared_data, target_vis_id, dest_vis_id):
        ...
        self.target_debug_line = None # Added to store target line ID
    ...
    def update_target_line(self, base_pos):
        ...
        if self.target_debug_line is not None:
            ...
            except p.error: pass # Ignore if already removed
            self.target_debug_line = None

        # Draw new line if robot has a valid target block AND is in an approach/align/drive state
        if self.target_block_id is not None and self.current_state >= STATE_APPROACH_BLOCK and self.current_state <= STATE_FINAL_DRIVE_BLOCK: # Only for block phase
            try:
                ...
                self.target_debug_line = None # Failed to get target pos or draw
    ...
    def run_step(self, all_robots):
        ...
        # --- Update target line visualization ---
        ...
        if self.current_state == STATE_IDLE:
            ...
            try:
             ...
             try:
                ...
                vec = np.array([base_pos[0]-pos_block[0], base_pos[1]-pos_block[1]]); norm = np.linalg.norm(vec); vec_norm = vec/norm if norm > 1e-3 else np.array([1,0])
                dot_x = np.dot(vec_norm, local_x); dot_y = np.dot(vec_norm, local_y)
                chosen_axis = local_x if abs(dot_x) >= abs(dot_y) else local_y; chosen_axis = chosen_axis if (dot_x >= 0 if abs(dot_x) >= abs(dot_x) else dot_y >= 0) else -chosen_axis
            ...
            # (Logic unchanged - checks destination lock)
            ...
            if distance > FINAL_APPROACH_DISTANCE:
                ...
                elif current_locker == self.robot_instance_id: # Already have lock
        ...
        # Check AFTER calculating desired velocities based on state logic
        # Only check if actively trying to move
        if abs(left_velocity) > 0.01 or abs(right_velocity) > 0.01:
             ...
             if should_stop_for_collision:
                  left_velocity = 0.0; right_velocity = 0.0
                  # Optional: Could add logic to maybe enter a temporary waiting state here
        ...
        # --- Update Target Line Visualization ---
        # This is handled by the main loop now
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    # <<< Initialize Shared Data including lock and new index >>>
    shared_data = {
        'placed_count': 0,
    ...
    shared_grid_map = create_grid_map() # Create the initial grid map

    # <<< Tkinter Grid Window Setup >>>
    # Create the Tkinter root and canvas
    ...
    try:
        while p.isConnected():
            ...
            # We need to know where things are for pathfinding/collision avoidance
            placed_block_ids = [id for id, status in block_states.items() if status == 'PLACED'] # Get IDs of placed blocks
            ...
            for i, bot in enumerate(robots):
                try:
                    current_camera_target = bot.run_step(robots) # Pass the list of robots
            ...
            try:
                 update_all_block_visualizations(robots[0].block_viz_dict) # Update block axis/sphere viz
                 if auto_camera:
            ...
            try:
                 ...
                 blocks_text = f"Placed: {shared_data['placed_count']}/{NUM_BLOCKS}"
            ...
            try:
                ...
                grid_window_root.update_idletasks() # Process Tkinter events
                grid_window_root.update() # Update the display
            except tk.TclError:
                # Handle window being closed by user
                print("[INFO] Tkinter window closed. Terminating simulation.")
                break # Exit the PyBullet loop
            ...
            if current_time - last_print_time >= 1.0:
                 ...
                 # Optional: Print a slice of the grid map for text debugging
                 # print("Grid Slice Y 0-10:\n", shared_grid_map[0:10, :])
                 print("-" * 20)
