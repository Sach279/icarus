...
SIMULATION_TIMESTEP = 1/120.0 # Physics timestep
NUM_BLOCKS = 5
NUM_ROBOTS = 2
...
DEBUG_DRAW_LIFETIME = 0.25 # <<< Increased lifetime for visibility on fast monitors
...
class BuilderBot:
            ...
            try:
                ...
                if are_valid_coordinates(base_pos, target_pos):
                    self.target_debug_line = p.addUserDebugLine(
                        base_pos, [target_pos[0], target_pos[1], base_pos[2]],
                        lineColorRGB=[0, 1, 0], lineWidth=2, lifeTime=DEBUG_DRAW_LIFETIME
                    )
    ...
    def run_step(self, all_robots):
        ...
        if self.current_state == STATE_IDLE:
            ...
            if self.shared_data.get('block_targets_assigned', False):
                ...
                if my_target_id != -1 and self.block_states.get(my_target_id) == 'AVAILABLE':
                    ...
                    try:
                        ...
                        else: pid_output = self.angle_pid.compute(angle_error, SIMULATION_TIMESTEP); left_velocity = -pid_output * ROTATION_SPEED_SCALE; right_velocity = pid_output * ROTATION_SPEED_SCALE; rotate_only = True # Use full scale
            ...
            # Point away from the destination for placement
            if math.hypot(dx, dy) < 0.01: desired_angle = rover_yaw # Avoid atan2(0,0)
...
def run_simulation(urdf_path, block_urdf_path):
    ...
    for i in range(NUM_ROBOTS):
        bot = BuilderBot(i, robot_ids[i], joint_dicts[i], block_visualizations, block_states, shared_data, target_vis_id, dest_body_id)
        robots.append(bot)
    ...
    try:
        while p.isConnected():
            ...
            camera_target_overall = None; active_bot_index = 0
            ...
            try:
                 ...
                 if auto_camera:
                     ...
                     if valid_active_bot_index < len(robots): active_bot_pos, _ = p.getBasePositionAndOrientation(robots[valid_active_bot_index].robot_id); update_camera_view(auto_camera, active_bot_pos, camera_target_overall)
                 if lidar_enabled:
                     for bot in robots: simulate_lidar(bot.robot_id)
                 draw_grid_map(grid_window_root.grid_data); grid_window_root.update_idletasks(); grid_window_root.update()
            except p.error: pass
            except IndexError: pass
            except tk.TclError: print("[INFO] Tkinter window closed. Terminating simulation."); break

            for i, bot in enumerate(robots):
                if bot.dock_constraint is not None:
                    try:
                        bot_base_pos, _ = p.getBasePositionAndOrientation(bot.robot_id); dest_pos = bot.current_placement_target
            ...
            p.stepSimulation()
            # If RealTimeSimulation(1), time.sleep controls the visual speed somewhat
            # Remove or adjust if using RealTimeSimulation(0)
...
if __name__ == "__main__":
    urdf_file = r"C:\Users\akshi\Documents\Building Block\Models\CubeBuilder.urdf" # Use your actual path
    block_urdf_file = r"C:\Users\akshi\Documents\Building Block\Models\CubeStructure.urdf" # Use your actual path
