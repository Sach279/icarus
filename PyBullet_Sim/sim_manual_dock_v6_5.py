...
def load_and_control_urdf(urdf_path):
    ...
    k = 4  # scaling factor for PID rotation
    ...
    try:
        while True:
            ...
            match state:
                ...
                case 1:  # FINAL_ALIGN_BLOCK (Relative Orientation Alignment)
                    # Get block's pose
                    pos_block, orn_block = p.getBasePositionAndOrientation(block_body_id)
                    ...
                    # Compute block's local x and y axes (horizontal only)
                    rot_mat = p.getMatrixFromQuaternion(orn_block)
                    local_x = np.array([rot_mat[0], rot_mat[3]])
                    local_y = np.array([rot_mat[1], rot_mat[4]])

                    # Compute horizontal vector from block to rover
                    ...
                    # Choose docking face: select the local axis (x or y) that best aligns with vec_norm
                    ...
                    print(
                        f"[DEBUG] FINAL_ALIGN_BLOCK: chosen_axis={chosen_axis}, desired_angle={math.degrees(desired_angle):.1f}°, rover_yaw={math.degrees(rover_yaw):.1f}°, angle_error={math.degrees(angle_error):.1f}°")
                    ...
                    if abs(angle_error) > stop_alignment_threshold:
                        ...
                        left_velocity = 0.0
                        right_velocity = 0.0
                        print("[DEBUG] FINAL_ALIGN_BLOCK: Relative orientation alignment complete.")
                        # Optionally, you can transition to driving forward or next stage:
                        stage = 4
